...
void LEDOutput::process(){
	...
	if (_state_fade_inprogress){
		_millis_now = millis();
		// If we have passed the end time, set the output to the target value
		if (_millis_now > _state_fade_end_millis){
			setDimPWMExact(_state_fade_pwm_target);
			// End the fade event
			_state_fade_inprogress = false;
			// Flag that the status callback should be called
			_status_update_needed = true;
		} 
		// Otherwise, calculate what the PWM output should be 
		else {
			// Calculate how many millis we have left to fade for
			float millis_remain = _state_fade_end_millis - _millis_now;
			// Calculate which PWM step we should be at
			uint16_t pwm_delta = _state_fade_pwm_target - int(millis_remain * _state_fade_pwmconst);
	...
	if (_state_autooff){
		_millis_now = millis();
		if (_millis_now > _state_autooff_millis){
			// Fade to 0, at 10 times the default fade time 
			setDimFadeStart(0, _state_fade_default_millis*10);
			_state_autooff = false;
		}
	}
	
	// Then, update the PWM output if needed
	// Check if the output needs updating
	if (_state_pwm != _state_pwm_last){
		led->setPWM(_state_pwm);
		_state_pwm_last =_state_pwm;
	...
	if (_status_update_needed){
		// Call the status callback
		if (_status_callback){
			_status_callback();
		}
		_status_update_needed = false;
	}
}
...
void LEDOutput::setDimStep(uint8_t inDimStep){
	...
	if (inDimStep > NUM_DIM_STEPS-1){
		_state_dim_level_goal = NUM_DIM_STEPS-1;
	} 
	else if (inDimStep < 0){
		_state_dim_level_goal = 0;
	}
	else {
		_state_dim_level_goal = inDimStep;
	}
	// Set the PWM to the value corresponding to this dim level
	setDimPWM(pwm_dim_levels[_state_dim_level_goal]);
	// Note that setDimPWM() will set _state_dim_level by
...
void LEDOutput::setDimStepUp(){
	...
	// Check for sanity
	if(_state_dim_level_goal >= NUM_DIM_STEPS){
		_state_dim_level_goal = NUM_DIM_STEPS-1;
	} 
	_millis_now = millis();
	switch (_state_dim_level_goal){
		...
		case NUM_DIM_STEPS-2:
		{
			...
			if (_millis_now > _state_lockout_end_millis){
				// Lockout is over, increment dim step 
				_state_dim_level_goal++; 
				// Set the PWM to the value corresponding to this dim level
				setDimPWM(pwm_dim_levels[_state_dim_level_goal]);
			} else {
				// Lockout still applies, reset counter
				_state_lockout_end_millis = _millis_now + _state_step_lockout_millis; 
			}
			break;
		}
		default:
...
void LEDOutput::setDimPWM(uint16_t inPWM){
	...
	_sane_pwm = _sane_in_pwm(inPWM);
	int closest_step = _find_closest_step(_sane_pwm);
	setDimFadeStart(pwm_dim_levels[closest_step], _state_fade_default_millis);
}
...
void LEDOutput::setDimPWMExact(uint16_t inPWM){
	...
	#if NUM_DIM_STEPS>1
		_state_pwm = _sane_in_pwm(inPWM);
		// If _state_pwm is less than 3, _state_percent would truncate to 0
		if (_state_pwm < 3 && _state_pwm > 0){
			//  Instead, set it to 1%
			_state_percent = 1;
		} 
		else {
			_state_percent = int((100*_state_pwm)/MAX_PWM);
		}
		_state_dim_level = _find_closest_step(_state_pwm);
...
void LEDOutput::setDimFadeStart(uint16_t inTargetPWM, uint16_t inTimeMillis){
	...
	_state_fade_pwm_target = _sane_in_pwm(inTargetPWM);
	// Also update the target dim level
	_state_dim_level_goal = _find_closest_step(_state_fade_pwm_target);
	...
	if (inTimeMillis == 0) {
		...
		_state_fade_end_millis = millis() + inTimeMillis;
		// Calculate the rate at which we need to change the PWM output
		_state_fade_pwmconst = (_state_fade_pwm_target - _state_pwm);
		_state_fade_pwmconst = _state_fade_pwmconst / inTimeMillis;
		// Flag that we are in a dimming cycle
		_state_fade_inprogress = true;
	}
}
...
void LEDOutput::setAutoOffTimer(uint32_t inTimeMillis){
	...
	_state_autooff = true; 
	_state_autooff_millis = millis() + inTimeMillis;
}
...
uint8_t LEDOutput::getDimPercent(){
	return _state_percent;
}

uint8_t LEDOutput::getDimStep(){
	return _state_dim_level;
}

uint8_t LEDOutput::getDimDefaultFade(){
	return _state_fade_default_millis;
}
...
// Private methods
uint8_t LEDOutput::_find_closest_step(uint16_t inPWM){
	/// Find the nearest PWM step to the given input
	int closest_step = 0;
	_sane_pwm = _sane_in_pwm(inPWM);
	for (int idx=0; idx<NUM_DIM_STEPS; idx++){
		if (_sane_pwm <= pwm_dim_levels[idx]){
			closest_step = idx;
			break;
		}
	}
...
uint16_t LEDOutput::_sane_in_pwm(uint16_t inPWM){
